...
COMPOSE="sudo docker compose -f docker-compose.yml -f docker-compose.staging.yml"
GATEWAY_URL="${GATEWAY_URL:-http://localhost:8080}"

cleanup() {
  echo
  echo "Stopping staging-like stack..."
  $COMPOSE down
}

trap cleanup EXIT

echo "Starting staging-like stack..."
$COMPOSE up -d --build
...
for i in {1..30}; do
  ...
  if [ "$STATUS" = "200" ] || [ "$STATUS" = "304" ]; then
    echo "Staging-like gateway check passed: HTTP ${STATUS}"
    exit 0
  fi
...
echo "Staging-like gateway check failed."
echo "Gateway logs:"
