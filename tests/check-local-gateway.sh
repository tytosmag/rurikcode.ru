...
echo "Checking that gateway points to Vite dev server port..."
$COMPOSE exec gateway sh -c "cat /etc/nginx/nginx.conf | grep -q 'server client:5173;'"
...
for i in {1..30}; do
  ...
  if [ "$STATUS" = "200" ] || [ "$STATUS" = "304" ]; then
    echo "Local dev gateway check passed: HTTP ${STATUS}"
    exit 0
  fi
...
echo "Local dev gateway check failed."
echo "Gateway logs:"
